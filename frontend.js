...
function makeUI()
{
    ...
    italic.appendChild(document.createTextNode('Clique no nome de usuário para gerar um ID aleatório'));
...
function activateAccount(idx)
{
    ...
    for(var i = 7; i >= 0; i--)
    {
        ...
        if(digit != digit)
        {
            alert("Deve ter 16 caracteres e conter apenas [0-9A-F]");
            return;
        }
    ...
    if(ui[idx].id.value == acc_id.toUpperCase())
        alert("ID PSN definido com sucesso!");
    return false;
}
